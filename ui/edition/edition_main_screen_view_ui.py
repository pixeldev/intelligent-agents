...
class EditionMainScreenViewUi(ViewUi):
  def __init__(self, view_service: ViewUiService):
    super().__init__(ViewUiConstants.EDITION_MAIN_SCREEN_IDENTIFIER)
    self.__view_service = view_service

  def create_control(self) -> list[flet.Control]:
    return self.__view_service.create_control(ViewUiConstants.EDITION_MAP_SELECTION_SCREEN_IDENTIFIER)
